package com.kaka.rockerlibrary.view;
...
import com.kaka.bluetoothble_client.R;
import com.kaka.rockerlibrary.Logger;
...
public class RockerView extends View {
    ...
    private OnShakeListener mOnShakeListener1;

    private DirectionMode mDirectionMode;
    private Direction tempDirection = Direction.DIRECTION_CENTER;
    private DirectionUD tempDirectionUD= DirectionUD.DIRECTIONUD_UP;
    // 角度
    ...
    private void callBackStart() {
        ...
        if (null != mOnShakeListener1) {
            mOnShakeListener1.onStart();
        }
    }
            ...
            if (CallBackMode.CALL_BACK_MODE_MOVE == mCallBackMode) {
                ...
                switch (mDirectionMode) {
                    ...
                    case DIRECTION_4_ROTATE_45:// 四个方向 旋转45度
                        ...
                        break;
                    case DIRECTION_6_ROTATE:// 6个方向

                        if ((ANGLE_VERTICAL_2D_OF_1P <= angle && ANGLE_360 > angle) && tempDirectionUD != DirectionUD.DIRECTIONUD_UP){
                            // 前
                            tempDirectionUD = DirectionUD.DIRECTIONUD_UP;
                            mOnShakeListener.directionUD(DirectionUD.DIRECTIONUD_UP);

                        }else if((ANGLE_0 <= angle && ANGLE_VERTICAL_2D_OF_1P > angle) && tempDirectionUD != DirectionUD.DIRECTIONUD_DOWN)
                        {
                            // 后
                            tempDirectionUD = DirectionUD.DIRECTIONUD_DOWN;
                            mOnShakeListener.directionUD(DirectionUD.DIRECTIONUD_DOWN);
                        }
                        if ((ANGLE_0 <= angle && ANGLE_ROTATE45_4D_OF_0P > angle || ANGLE_ROTATE45_4D_OF_3P <= angle && ANGLE_360 > angle) && tempDirection != Direction.DIRECTION_RIGHT) {
                            // 右
                            tempDirection = Direction.DIRECTION_RIGHT;
                            mOnShakeListener.direction(Direction.DIRECTION_RIGHT);
                        } else if ((ANGLE_ROTATE45_4D_OF_0P <= angle && ANGLE_ROTATE45_4D_OF_1P > angle||ANGLE_ROTATE45_4D_OF_2P <= angle && ANGLE_ROTATE45_4D_OF_3P > angle) && tempDirection != Direction.DIRECTION_Go_straight) {
                            //直行
                            tempDirection = Direction.DIRECTION_Go_straight;
                            mOnShakeListener.direction(Direction.DIRECTION_Go_straight);
    ...
    public interface OnShakeListener {
        ...
        void directionUD(DirectionUD direction);
        // 结束
        void onFinish();
    }
